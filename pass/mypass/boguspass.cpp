  ...
  struct BogusControlFlow : public FunctionPass {
            ...
            for(int random = (int)llvm::cryptoutils->get_range(10); random < 10; ++random){
              switch(llvm::cryptoutils->get_range(3)){ // can be improved
                ...
                case 1: op = BinaryOperator::Create(Instruction::FSub,
                            i->getOperand(0),
                            i->getOperand(1),*var,&*i);    //TODO: Used to be CreateFNeg(i->getOperand(0),*var,&*i);
    ...
    bool doF(Module &M){
      ...
      for(std::vector<Instruction*>::iterator i =toEdit.begin();i!=toEdit.end();++i){
        //if y < 10 || x*(x+1) % 2 == 0
        opX = new LoadInst (Type::getInt32Ty(M.getContext()), (Value *)x, "", (*i)); // didnt have type argument before, added same type as x
